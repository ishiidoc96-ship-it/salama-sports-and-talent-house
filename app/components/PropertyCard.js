    ...
    <Link href={`/properties/${property.id}`} className="card-property group block">
      <div className="relative aspect-[4/3] overflow-hidden">
        ...
        <span
          className={`absolute top-3 left-3 px-3 py-1 text-xs font-semibold rounded-full ${
            categoryColors[property.category] || "bg-brand-stone-800 text-white"
          }`}
        >
        ...
        {property.demo && (
          <span className="absolute top-3 right-3 px-2 py-1 text-[10px] font-medium rounded bg-black/40 text-white/80 backdrop-blur-sm">
            DEMO
          </span>
        )}

        <div className="absolute bottom-3 left-3 right-3">
          <h3 className="text-white font-serif font-semibold text-lg leading-tight mb-1">
            {property.title}
          </h3>
      ...
      <div className="p-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-brand-stone-500 text-xs font-medium uppercase tracking-wide">
            {property.type}
          </span>
        </div>
        ...
        <div className="flex items-center justify-between pt-3 border-t border-brand-stone-100">
          <span className="text-brand-stone-500 text-xs">
            {property.address}
          </span>
          <span className="flex items-center gap-1 text-brand-gold text-sm font-medium group-hover:gap-2 transition-all">
            View Details
